...
impl<'a> AgentRunner<'a> {
    ...
    pub fn abort(&self, state: &mut AgentState) -> Result<AgentOutcome> {
    ...
    async fn run_loop(&self, state: &mut AgentState, resumed: bool) -> Result<AgentOutcome> {
        ...
        if let Err(error) = self.emit_mcp_manifest() {
            return self.fail(state, error);
        }
        if resumed && let Err(error) = self.recover_in_flight_call(state) {
            return self.fail(state, error);
        }
        ...
        while state.step < state.max_steps {
            ...
            let response = tokio::select! {
                biased;
                _ = self.cancellation.cancelled() => return self.abort(state),
                response = self.client.chat(built.request) => match response {
    ...
    fn emit_mcp_manifest(&self) -> Result<()> {
        let names = self.tools.mcp_tool_names();
        let mut servers = std::collections::BTreeMap::<&str, usize>::new();
        for name in &names {
            let (server, _) =
                parse_namespaced_name(name).expect("registry returned only validated MCP names");
            *servers.entry(server).or_default() += 1;
        }
        for (server, tool_count) in servers {
            self.emit(TraceEventKind::McpServerConnected {
        ...
        for name in names {
            let (server, _) =
                parse_namespaced_name(&name).expect("registry returned only validated MCP names");
            self.emit(TraceEventKind::McpToolRegistered {
                server: server.to_owned(),
                name,
            })?;
        }
    ...
    fn recover_in_flight_call(&self, state: &mut AgentState) -> Result<()> {
        let Some(call) = state.in_flight_tool_call.clone() else {
            return Ok(());
        };
        let observation = format!(
            "Tool execution outcome is indeterminate after interruption; `{}` was not replayed. Reconcile external state before requesting another call.",
            call.name
        );
        state
            .messages
    ...
    async fn policy_checked_observation(
        ...
        if let Some((server, _)) = parse_namespaced_name(&call.name) {
            self.emit(TraceEventKind::McpCallResult {
                server: server.to_owned(),
                name: call.name.clone(),
                success: observation.success,
            })?;
    ...
    async fn execute_observation(&self, state: &mut AgentState, call: &ToolCall) -> Observation {
        let side_effectful = !matches!(call.name.as_str(), "read_file" | "list_files");
        if side_effectful {
            state.in_flight_tool_call = Some(crate::agent::InFlightToolCall {
                id: call.id.clone(),
                name: call.name.clone(),
                side_effectful,
            });
            if let Err(error) = self.checkpoint(state) {
                return Observation::unbounded(
                    format!(
                        "Tool execution was not started because checkpointing failed: {error:#}"
                    ),
                    false,
                );
            }
...
mod resume_context_tests {
    ...
    #[tokio::test]
    async fn unavailable_prior_mcp_tool_becomes_failed_observation_on_resume() {
        let client = FinalClient(Mutex::new(Vec::new()));
        let tools = ToolRegistry::new();
        let trace = Trace::default();
        let runner = AgentRunner::with_persistence(
        ...
        assert!(requests[0].messages.iter().any(|message| {
            message.role == crate::llm::Role::Tool
                && message
                    .content
                    .as_deref()
                    .is_some_and(|content| content.contains("unknown tool"))
        }));
        assert!(
            trace
                .0
                ...
                .unwrap()
                .iter()
                .any(|event| matches!(event, TraceEventKind::McpCallResult { success: false, .. }))
        );
    }
}
