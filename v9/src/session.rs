...
pub struct Session {
    ...
    /// Non-secret capability marker used to rebuild the task-scoped V9 registry on resume.
...
impl SessionRepository for FileSessionRepository {
    ...
    fn load(&self, id: Uuid) -> Result<Session> {
        ...
        if envelope.version != SESSION_SCHEMA_VERSION {
            bail!(
                "unsupported session schema version {}; V9 requires version {SESSION_SCHEMA_VERSION} and does not migrate older sessions",
                envelope.version
            );
        }
        let session: Session = serde_json::from_slice(&bytes)
            .context("session file contains invalid V9 session JSON")?;
...
fn validate_session(session: &Session, expected_id: Uuid, workspace: &Path) -> Result<()> {
    ...
    if !session.sub_agent_enabled {
        bail!("V9 session does not contain the required sub-agent capability marker");
    }
...
mod tests {
    ...
    fn roundtrip_preserves_summary_and_settings() {
        ...
        assert!(session.sub_agent_enabled);
    ...
    #[test]
    fn requires_sub_agent_capability_marker() {
        let w = TestWorkspace::new("missing-capability");
        fs::create_dir(w.path().join("traces")).unwrap();
        let repo = FileSessionRepository::new(w.path()).unwrap();
        ...
        repo.create(&session).unwrap();
        let mut value = serde_json::to_value(session).unwrap();
        value.as_object_mut().unwrap().remove("sub_agent_enabled");
        fs::write(repo.session_path(id), serde_json::to_vec(&value).unwrap()).unwrap();
        assert!(format!("{:#}", repo.load(id).unwrap_err()).contains("invalid V9 session JSON"));
    }
}
