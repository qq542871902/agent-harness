    ...
    pub fn new(config: &Config) -> Result<Self> {
        ...
        if base_url.scheme() != "https" {
            ...
            if base_url.scheme() != "http" || !config.allow_http_loopback || !loopback {
                bail!(
                    "OPENAI_BASE_URL must use HTTPS; plain HTTP requires ALLOW_HTTP_LOOPBACK=true and an explicit loopback host"
                );
            }
        }
...
mod tests {
    ...
    #[test]
    fn rejects_non_https_unless_loopback_is_explicitly_enabled() {
        assert!(OpenAiCompatibleClient::new(&config("file:///tmp/provider")).is_err());
        assert!(OpenAiCompatibleClient::new(&config("http://example.test/v1")).is_err());
        assert!(OpenAiCompatibleClient::new(&config("http://127.0.0.1:8080/v1")).is_err());

        let mut loopback = config("http://127.0.0.1:8080/v1");
        loopback.allow_http_loopback = true;
        assert!(OpenAiCompatibleClient::new(&loopback).is_ok());
        loopback.base_url = "http://[::1]:8080/v1".into();
        assert!(OpenAiCompatibleClient::new(&loopback).is_ok());
        loopback.base_url = "http://example.test/v1".into();
        assert!(OpenAiCompatibleClient::new(&loopback).is_err());
