...
impl Tool for ListFilesTool {
    ...
    async fn execute(&self, arguments: Value) -> Result<ToolOutput> {
            ...
            .with_context(|| format!("failed to list directory `{}`", directory.display()))?
        {
            ...
            if super::tool::is_sensitive_path(relative) {
                continue;
            }
...
mod sensitive_path_tests {
    ...
    #[tokio::test]
    async fn hides_sensitive_entries_and_rejects_sensitive_directories() {
        let w = TestWorkspace::new("list-sensitive");
        fs::write(w.path().join("visible.txt"), "ok").unwrap();
        fs::write(w.path().join(".env"), "secret").unwrap();
        fs::write(w.path().join(".env.example"), "safe").unwrap();
        fs::create_dir(w.path().join(".sessions")).unwrap();
        fs::create_dir(w.path().join("traces")).unwrap();
        let tool = ListFilesTool::new(w.path()).unwrap();
        let listing = tool.execute(json!({"path":"."})).await.unwrap().content;
        assert_eq!(listing, ".env.example\nvisible.txt");
        assert!(tool.execute(json!({"path":".sessions"})).await.is_err());
        assert!(tool.execute(json!({"path":"traces"})).await.is_err());
    }
}
