...
/// Port used by the runner whenever policy returns `Ask`.
pub trait ApprovalHandler: Send + Sync {
    fn request_approval(&self, call: &ToolCall) -> Result<ApprovalDecision>;
}

/// Task-scoped approval overrides. Policy denial is evaluated before this state.
...
impl ApprovalHandler for ConsoleApprovalHandler {
    fn request_approval(&self, call: &ToolCall) -> Result<ApprovalDecision> {
        ...
        request_approval_from(&mut stdin.lock(), &mut stdout.lock(), call)
    }
}
...
fn request_approval_from(
    ...
    let arguments = redact_sensitive_values(&call.arguments);
...
mod tests {
    ...
    fn loops_until_grant_deny_or_always_and_formats_redacted_json() {
        for (input, expected) in [
            ...
            let decision = request_approval_from(&mut reader, &mut output, &call()).unwrap();
    ...
    fn always_allow_state_is_explicit_and_task_local() {
        ...
        assert!(state.is_always_allowed("write_file"));
        assert!(!ApprovalState::default().is_always_allowed("write_file"));
    }
}
