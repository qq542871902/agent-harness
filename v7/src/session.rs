use crate::{agent::AgentState, context::ContextSettings};
...
impl SessionRepository for FileSessionRepository {
    ...
    fn load(&self, id: Uuid) -> Result<Session> {
        ...
        if envelope.version != SESSION_SCHEMA_VERSION {
            bail!(
                "unsupported session schema version {}; V7 requires version {SESSION_SCHEMA_VERSION} and does not migrate older sessions",
                envelope.version
            );
        }
        let session: Session = serde_json::from_slice(&bytes)
            .context("session file contains invalid V7 session JSON")?;
...
mod tests {
    ...
    fn roundtrip_preserves_summary_and_settings() {
        ...
        assert!(!serde_json::to_string(&session).unwrap().contains("api_key"));
    }
    #[test]
