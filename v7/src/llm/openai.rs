...
mod transport_security_tests {
    ...
    fn requires_https_except_explicit_loopback_opt_in() {
        let mut remote = config("http://example.test/v1");
        remote.allow_insecure_loopback = true;
        assert!(OpenAiCompatibleClient::new(&remote).is_err());
        let mut local = config("http://localhost:1234/v1");
        assert!(OpenAiCompatibleClient::new(&local).is_err());
        local.allow_insecure_loopback = true;
        assert!(OpenAiCompatibleClient::new(&local).is_ok());
        let mut ipv6 = config("http://[::1]:1234/v1");
        ipv6.allow_insecure_loopback = true;
        assert!(OpenAiCompatibleClient::new(&ipv6).is_ok());
    }
}
