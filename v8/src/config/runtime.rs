...
impl Config {
    /// Loads provider secrets plus optional, validated V8 safety and context settings.
    pub fn from_env() -> Result<Self> {
        dotenvy::dotenv().ok();
        let api_key = required_env("OPENAI_API_KEY")?;
        let base_url = required_env("OPENAI_BASE_URL")?;
        let allow_insecure_loopback = optional_bool_env("OPENAI_ALLOW_INSECURE_LOOPBACK")?;
...
fn optional_bool_env(name: &str) -> Result<bool> {
    match optional_env(name)?.as_deref().map(str::trim) {
        None | Some("") | Some("0" | "false") => Ok(false),
        Some("1" | "true") => Ok(true),
