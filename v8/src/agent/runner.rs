...
impl<'a> AgentRunner<'a> {
    ...
    fn recover_in_flight_call(&self, state: &mut AgentState) -> Result<()> {
        let Some(call) = state.in_flight_tool_call.take() else {
            return Ok(());
        };
        let position = state
            .pending_tool_calls
            ...
            .context("persisted in-flight tool call is not pending")?;
        let observation = Observation::unbounded(
            format!(
                "Tool execution was interrupted and its outcome is unknown. For safety, `{}` was not automatically replayed; inspect state before explicitly retrying.",
                call.name
            ),
            false,
        );
        ...
        state
            .messages
            .push(Message::tool_result(call.id, observation.content));
        state.pending_tool_calls.remove(position);
