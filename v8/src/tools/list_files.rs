use super::{
    ...
    tool::{
        ...
        is_sensitive_workspace_path, resolve_existing_path,
    },
};
...
impl Tool for ListFilesTool {
    ...
    async fn execute(&self, arguments: Value) -> Result<ToolOutput> {
            ...
            .with_context(|| format!("failed to list directory `{}`", directory.display()))?
        {
            ...
            if is_sensitive_workspace_path(&self.workspace, &path) {
                continue;
            }
            if let Ok(resolved) = path.canonicalize()
                && is_sensitive_workspace_path(&self.workspace, &resolved)
            {
                continue;
            }
            entries.push(
