...
use mini_harness_v9::{
    ...
    policy::{ApprovalHandler, ConsoleApprovalHandler, Policy},
...
use retrieval::{V10Policy, WorkspaceSearchTool};
...
async fn main() -> Result<()> {
    ...
    let rag_config = RagConfig::from_env(&config)?;
    let sensitive_values = rag_config.sensitive_values(&config.api_key);
    let client: Arc<dyn LlmClient> = Arc::new(OpenAiCompatibleClient::new(&config)?);
    let workspace = workspace_directory()?;
    let mut registry = build_native_tool_registry(&workspace, &rag_config)?;
    ...
    let policy: Arc<dyn Policy> = Arc::new(V10Policy);
...
fn build_native_tool_registry(workspace: &Path, rag_config: &RagConfig) -> Result<ToolRegistry> {
    ...
    registry.register(WorkspaceSearchTool::with_config(
        workspace,
        rag_config.clone(),
    )?)?;
    Ok(registry)
...
async fn run_interactive(
    ...
    println!("mini-harness V10 controlled workspace retrieval agent — type /exit to quit.");
    let stdin = io::stdin();
...
async fn run_task(
    ...
    let runner = AgentRunner::with_persistence(
        ...
        policy.as_ref(),
        approver.as_ref(),
        ContextBuilder::with_sensitive_values(config.context.clone(), sensitive_values.to_vec())?,
        RunPersistence {
            session_id: id,
            trace: writer.as_ref(),
            sessions: &checkpoint,
...
async fn resume_task(
    ...
    if !session.mcp_tools.is_empty() && !mcp_configured {
        bail!(
            "session {id} used MCP tools; set MCP_CONFIG_PATH to a current V10 MCP configuration before resume"
        );
    }
    ...
    let runner = AgentRunner::with_persistence(
        ...
        policy.as_ref(),
        approver.as_ref(),
        ContextBuilder::with_sensitive_values(settings, sensitive_values.to_vec())?,
        RunPersistence {
            session_id: id,
            trace: writer.as_ref(),
            sessions: &checkpoint,
...
fn usage() -> &'static str {
    "Usage:\n  mini-harness-v10\n  mini-harness-v10 run <prompt>\n  mini-harness-v10 resume <session-id>\n  mini-harness-v10 session <session-id>\n  mini-harness-v10 trace <session-id>\n\nV10 adds search_workspace_knowledge with lexical, vector, or hybrid retrieval selected by RAG_MODE. Lexical mode is local and is the default; vector modes require EMBEDDING_MODEL and send bounded non-sensitive workspace chunks to the configured OpenAI-compatible embeddings endpoint. Results are read-only citations, and traces retain only normal payload-free tool metadata. V10 reuses the V9 session schema, so do not mix V9 and V10 resumptions for the same incomplete session."
}
