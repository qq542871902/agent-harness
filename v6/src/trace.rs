...
impl JsonlTraceWriter {
    ...
    /// Opens and validates the existing trace before appending resumed events.
    pub fn open_append(workspace: &Path, session_id: Uuid) -> Result<Self> {
        let path = resolve_trace_path(workspace, session_id)?;
        let reader = BufReader::new(File::open(&path).context("failed to open existing trace")?);
...
mod tests {
    ...
    #[test]
    fn append_mode_validates_and_extends_the_same_trace() {
        let workspace = TestWorkspace::new("trace-append");
        let session_id = Uuid::from_u128(43);
        let writer = JsonlTraceWriter::create(workspace.path(), session_id).unwrap();
        writer
            .record(&TraceEvent::new(session_id, TraceEventKind::SessionStarted))
            .unwrap();
        drop(writer);

        let writer = JsonlTraceWriter::open_append(workspace.path(), session_id).unwrap();
        writer
            .record(&TraceEvent::new(session_id, TraceEventKind::SessionResumed))
            .unwrap();
        let events: Vec<TraceEvent> = fs::read_to_string(writer.path())
            .unwrap()
            .lines()
            .map(|line| serde_json::from_str(line).unwrap())
            .collect();
        assert_eq!(events.len(), 2);
        assert_eq!(events[1].kind, TraceEventKind::SessionResumed);
    }
    ...
    fn trace_resolution_rejects_symlink_escape() {
        ...
        let same_directory_target = trace_directory.join("alternate.jsonl");
        fs::write(&same_directory_target, "{}\n").unwrap();
        let same_directory_id = Uuid::from_u128(10);
        symlink(
            &same_directory_target,
            trace_path(&trace_directory, same_directory_id),
        )
        .unwrap();
        assert!(resolve_trace_path(workspace.path(), same_directory_id).is_err());
        assert!(JsonlTraceWriter::open_append(workspace.path(), same_directory_id).is_err());
    }
