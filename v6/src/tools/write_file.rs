...
mod tests {
    ...
    async fn reserved_harness_subtrees_cannot_be_replaced() {
        ...
        let session_directory = workspace.path().join(".sessions");
        fs::create_dir(&session_directory).unwrap();
        let session = session_directory.join("active.json");
        fs::write(&session, "original session\n").unwrap();
        ...
        assert!(
            tool.execute(json!({"path": ".sessions/active.json", "content": "replaced"}))
                .await
                .is_err()
        );
        assert_eq!(fs::read_to_string(trace).unwrap(), "original\n");
        assert_eq!(fs::read_to_string(session).unwrap(), "original session\n");
    }
}
