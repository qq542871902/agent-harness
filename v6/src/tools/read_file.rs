...
use super::{
    Tool, ToolOutput,
    tool::{MAX_FILE_BYTES, canonical_workspace, ensure_not_sensitive, resolve_existing_path},
};
...
impl Tool for ReadFileTool {
    ...
    async fn execute(&self, arguments: Value) -> Result<ToolOutput> {
        ...
        ensure_not_sensitive(&self.workspace, &path)?;
        let metadata = path
            .metadata()
...
mod sensitive_path_tests {
    ...
    #[tokio::test]
    async fn denies_sensitive_files_and_allows_env_example() {
        let workspace = TestWorkspace::new("read-sensitive");
        fs::write(workspace.path().join(".env"), "SECRET=value").unwrap();
        fs::write(workspace.path().join(".env.local"), "SECRET=value").unwrap();
        fs::write(workspace.path().join(".env.example"), "SECRET=").unwrap();
        fs::create_dir(workspace.path().join("credentials")).unwrap();
        fs::write(workspace.path().join("credentials/token"), "secret").unwrap();
        let tool = ReadFileTool::new(workspace.path()).unwrap();

        assert!(tool.execute(json!({"path": ".env"})).await.is_err());
        assert!(tool.execute(json!({"path": ".env.local"})).await.is_err());
        assert!(
            tool.execute(json!({"path": "credentials/token"}))
                .await
                .is_err()
        );
        assert_eq!(
