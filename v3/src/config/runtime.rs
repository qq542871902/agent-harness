...
pub struct Config {
    ...
    pub base_url: String,
    pub model: String,
    /// Allows plaintext HTTP only when the provider host is loopback.
...
impl Config {
    ...
    pub fn from_env() -> Result<Self> {
        ...
        let allow_loopback_http = env_flag("OPENAI_ALLOW_LOOPBACK_HTTP")?;
...
/// Parses an optional boolean opt-in flag. Absent or empty means false.
fn env_flag(name: &str) -> Result<bool> {
    match env::var(name) {
        Ok(value) => match value.trim().to_ascii_lowercase().as_str() {
            "" | "0" | "false" | "no" | "off" => Ok(false),
            "1" | "true" | "yes" | "on" => Ok(true),
            other => bail!("{name} must be a boolean flag, got `{other}`"),
        },
